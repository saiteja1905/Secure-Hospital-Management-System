...
function AdminTransactions(){
    ...
    const [pendTransaction, setPendTransactions] = useState([]);
    const [apprTransaction, setApprTransactions] = useState([]);
    const [rejTransaction, setRejTransactions] = useState([]);
    ...
    async function apptUpdate (apptId,accept) {
        const getappts = httpsCallable(functions, 'setTransactions');
        getappts({transactionId:apptId,status:accept})
            .then((result) => {
    ...
    const apptOperation = (apptid,accept) => {
        if(!disableAll[0]) {
             setdisAll([true,"Updating the transaction"]);
    ...
    async function loadTransactions () {
        const getTransactionInfo = httpsCallable(functions, 'getTransactions');
        getTransactionInfo({})
            .then((result) => {
                if(result.data.success) {
                    console.log(result.data.data)
                    const transactionRequests = result.data.data;
                    let final_trans = []
                    let pen_trans = []
                    let appr_trans = []
                    let rej_trans = []
                    for(var transaction in transactionRequests){
                        var trans = {}
                        trans["id"] = transaction;
                        trans["name"] = transactionRequests[transaction].name;
                        trans["patient_id"] = transactionRequests[transaction].patient_id;
                        trans["status"] = transactionRequests[transaction].status;
                        trans["report_id"] = transactionRequests[transaction].report_id;
                        trans["amount"] = transactionRequests[transaction].amount;;
                        var t = new Date(transactionRequests[transaction].date);
                        trans["d"] = t.getDate();
                        trans["y"] = t.getFullYear();
                        trans["m"] = month[t.getMonth()];
                        if (trans.status == 0){
                            pen_trans.push(trans);
                        } else if(trans.status == 1) {
                            appr_trans.push(trans);
                        } else if(trans.status == 2) {
                            rej_trans.push(trans);
                        }
                    }
                    console.log(final_trans)
                    setPendTransactions(pen_trans);
                    setApprTransactions(appr_trans);
                    setRejTransactions(rej_trans);
                    setdisAll([false,""]);
                }
            }).catch((error) => {
                console.log("Error fetching transaction records!");
            })
    }
                                ...
                                <div className="card">
                                    <div className="card-body pt-0">
                                        <nav className="user-tabs mb-4">
                                            <ul className="nav nav-tabs nav-tabs-bottom nav-justified">
                                                <li className="nav-item">
                                                    {tab.pen_trans
                                                        ? <a className="nav-link active"  data-toggle="tab" onClick={e=>toggleTab('pen_trans')}>Pending Transactions</a>
                                                        : <a className="nav-link"  data-toggle="tab" onClick={e=>toggleTab('pen_trans')}>Pending Transactions</a>}
                                                </li>
                                                <li className="nav-item">
                                                    {tab.appr_trans
                                                        ? <a className="nav-link active"  data-toggle="tab" onClick={e=>toggleTab('appr_trans')}>Approved Transactions</a>
                                                        : <a className="nav-link"  data-toggle="tab" onClick={e=>toggleTab('appr_trans')}>Approved Transactions</a>}
                                                </li>
                                                <li className="nav-item">
                                                    {tab.rej_trans
                                                        ? <a className="nav-link active" data-toggle="tab" onClick={e=>toggleTab('rej_trans')}>Rejected Transactions</a>
                                                        : <a className="nav-link" data-toggle="tab" onClick={e=>toggleTab('rej_trans')}>Rejected Transactions</a>}
                                                </li>
                                            </ul>
                                        </nav>
                                        ...
                                        <div className="tab-content pt-0">
                                            {tab.pen_trans &&
                                                <div id="pat_pen_trans" className="tab-pane fade show active">
                                                        ...
                                                        <div className="card-body">
                                                            <div className="table-responsive">
                                                                <table className="table table-hover table-center mb-0">
                                                                    ...
                                                                    <tbody>
                                                                    {
                                                                        pendTransaction.map(trans => (
                                                                            <tr>
                                                                                <td>
                                                                                    <h2 className="table-avatar">
                                                                                    <a onClick={() => history.push("/PatientProfile/" + trans.patient_id)}>{trans.patient_id}</a>
                                                                                    </h2>
                                                                                </td>
                                                                                <td>{trans.d} {trans.m} {trans.y}</td>
                                                                                <td><span
                                                                                className={statusToClass[trans.status].class}>{statusToClass[trans.status].text}</span>
                                                                                </td>
                                                                                <td>{trans.id}</td>
                                                                                <td>
                                                                                    <a onClick={() => history.push("/viewDiagnosis_doctor/" + trans.report_id)}>{trans.report_id}</a>
                                                                                </td>
                                                                                <td>
                                                                                    <a>{trans.amount}</a>
                                                                                </td>
                                                                                <td className="text-right">
                                                                                    <div className="table-action">
                                                                                        ...
                                                                                        <a onClick={() => apptOperation(trans.id,1)}
                                                                                           className="btn btn-sm bg-success-light">
                                                                                            <i className="fas fa-check"></i> Accept
                                                                                        </a>
                                                                                        <a onClick={() => apptOperation(trans.id,2)}
                                            ...
                                            {tab.appr_trans &&
                                                <div id="pat_appr_transs" className="tab-pane fade show active">
                                                <div className="card card-table mb-0">
                                                    <div className="card-body">
                                                        <div className="table-responsive">
                                                            <table className="table table-hover table-center mb-0">
                                            ...
                                            {tab.rej_trans &&
                                                <div id="pat_rej_trans" className="tab-pane fade show active">
                                                <div className="card card-table mb-0">
                                                    <div className="card-body">
                                                        <div className="table-responsive">
                                                            <table className="table table-hover table-center mb-0">
